...
import { Order, OrderItem, addOrder, updateOrder, fetchOrders } from '@/store/slices/orderSlice';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { toast } from 'sonner';
...
const OrderForm: React.FC<OrderFormProps> = ({ order, onClose }) => {
  ...
  const handleItemChange = (index: number, field: keyof OrderItem, value: any) => {
    ...
    if (field === 'productId') {
      ...
      if (selectedProduct) {
        item.productName = selectedProduct.product_name;
        item.price = selectedProduct.price_per_unit;
      }
    }
  ...
  const validate = () => {
    ...
    if (!formData.postalCode) errs.postalCode = 'Postal code is required';
    if (!formData.city) errs.city = 'City is required';
    if (!formData.state) errs.state = 'State is required';
    if (!formData.email || !/^\S+@\S+\.\S+$/.test(formData.email)) errs.email = 'Valid email required';
    if (!formData.mobileNumber || !/^[6-9]\d{9}$/.test(formData.mobileNumber)) errs.mobileNumber = 'Valid mobile number required';
    ...
    (formData.items || []).forEach((item, index) => {
      if (!item.productId) errs[`item-${index}-productId`] = 'Product required';
      if (item.quantity <= 0) errs[`item-${index}-quantity`] = 'Quantity > 0 required';
    });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (order) {
        await dispatch(updateOrder({id: order?.id!,  data: orderData})).unwrap();
  ...
  return (
    <div className="fixed inset-0 z-50 backdrop-blur-md bg-black/60 flex items-center justify-center">
      <div className="w-full max-w-4xl bg-gray-900 border border-blue-500 rounded-xl shadow-lg p-6 overflow-y-auto max-h-[90vh]">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-blue-300">{order ? 'Edit Order' : 'Create Order'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-blue-300 text-sm">Client *</label>
              ...
              {errors.customerId && <p className="text-red-400 text-xs">{errors.customerId}</p>}
            </div>

            <InputField label="Email" name="email" value={formData.email} onChange={handleChange} error={errors.email} />
            <InputField label="Phone" name="mobileNumber" value={formData.mobileNumber} onChange={handleChange} error={errors.mobileNumber} />
          </div>

          <div className="space-y-3">
            <h3 className="text-blue-300 font-medium">Items</h3>
            ...
            {(formData.items || []).map((item, index) => (
              <div key={index} className="grid grid-cols-12 gap-2 items-center">
                ...
                <div className="col-span-2"><Input type="number" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', e.target.value)} /></div>
                <div className="col-span-2"><Input type="number" value={item.price} onChange={(e) => handleItemChange(index, 'price', e.target.value)} /></div>
                <div className="col-span-2"><Input type="number" value={item.discount} onChange={(e) => handleItemChange(index, 'discount', e.target.value)} /></div>
                <div className="col-span-1 text-white">${item.total.toFixed(2)}</div>
                <div className="col-span-1 text-right">
                  <button type="button" onClick={() => removeItem(index)}><Trash2 className="text-red-500" /></button>
                </div>
              </div>
            ))}
            <Button type="button" onClick={addItem} className="mt-2"><Plus className="w-4 h-4 mr-1" /> Add Item</Button>
          </div>

          <div className="flex justify-end gap-4 pt-4 border-t border-gray-700 mt-6">
            <Button type="button" variant="outline" onClick={onClose}>Cancel</Button>
            <Button type="submit" className="bg-blue-600 hover:bg-blue-700">{order ? 'Update' : 'Create'} Order</Button>
          </div>
        </form>
      </div>
    </div>
...
const InputField = ({ label, name, value, onChange, error }: any) => (
  <div>
    ...
    <Input name={name} value={value} onChange={onChange} className="bg-gray-800 border-gray-700 text-white" />
